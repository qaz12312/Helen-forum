<?php
	...
	*/
    function doCreatAccount($input){
    	global $conn;
		$test=base64_encode($input["password"]);
        ...
        if($result->num_rows > 0){
            ...
            $sql="INSERT INTO  `Users`(`UserID`,`Password`,`Color`,`Nickname`) VALUES('".$input['account']."','".$test."','\#ffffff','".$input['account']."')";
            $resultNew=$conn->query($sql);
            if(!$resultNew){
                die($conn->error);
            }
            $sql="SELECT `UserID`,`Color`,`Nickname` FROM `Users` WHERE `UserID`='".$input['account']."' AND `Password`='".$test."'";
            $result=$conn->query($sql);
            if(!$result){
                die($conn->error);
            }
