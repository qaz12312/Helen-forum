        ...
        $sql="SELECT `Title`,`Start`,`END`,`UserID` FROM `Calendars` WHERE `ID`= ? AND `IsValid`= ? ";  
            ...
            if($resultCount <= 0){
                ...
                if($input['isPass']){ 
                    ...
                    doSendNotification(array("recipient" => $result[0][3], "content" => "Your activtiy has been added."));
                    ...
                    doSendNotification(array("recipient" => $result[0][3], "content" => "Your activtiy can't be added."));
